...
class Passenger:
	...
	def find_closest_node(self, pos):
		...
		for node in sample_nodes:
			current_distance = self.euclidean(float(node[0]), float(node[1]), float(pos[1]), float(pos[0]))
	...
	def calculate_min_pair(self, drivers, passengers):
		...
		for d_index, driver in enumerate(drivers):
			for p_index, passenger in enumerate(passengers):
				current_distance = self.euclidean(float(self.node_file[driver.closest_node_key]['lat']), float(self.node_file[driver.closest_node_key]['lon']), float(self.node_file[passenger.start_closest_node_key]['lat']), float(self.node_file[passenger.start_closest_node_key]['lon']))
				if current_distance < min_distance:
					min_driver = d_index
